...
-(NSString*)formatNineByteDoubleValueWithString:(NSString*)formatString
{
    ...
    uint D = responseFromAnyECU[3].unsignedIntValue;
    uint E = responseFromAnyECU[4].unsignedIntValue;
    uint F = responseFromAnyECU[5].unsignedIntValue;
    uint G = responseFromAnyECU[6].unsignedIntValue;
    uint H = responseFromAnyECU[7].unsignedIntValue;
    uint I = responseFromAnyECU[8].unsignedIntValue;
...
//    double adapted = offset + original * factor;
    ...
    if ( (A>>0&1) == 1 && A>>(0+4) == 0)
    {
        sensor1_1 = (double) (B * 256 + C);//big-endian
    }
    if ( (A>>1&1) == 1 && A>>(1+4) == 0)
    {
        sensor1_2 = (double) (D * 256 + E);
    }
    if ( (A>>2&1) == 1 && A>>(2+4) == 0)
    {
        sensor2_1 = (double) (F * 256 + G);
    }
    if ( (A>>3&1) == 1 && A>>(3+4) == 0)
    {
        sensor2_2 = (double) (H * 256 + I);
    }
    return [NSString stringWithFormat:formatString, sensor1_1, sensor1_2, sensor2_1, sensor2_2];
}
